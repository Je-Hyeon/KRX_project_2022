...
class MyKmeans:
    ...
    def find_optimal_k(
        ...
        Args:
            ...
            optimize_method ("inertia", "inter_std","silhouette"): 어떤 방식으로 k를 결정할지
    ...
    def find_optimal_initp(
        ...
        Args:
            ...
            optimize_method ("inertia", "inter_std","silhouette"): 어떤 방식으로 시작점을 결정할지
    ...
    @staticmethod
    def visualize_k_determine(res, method="inertia"):
        """
        Args:
            res: find_optimal_k에서 나온 결과
            method ("inertia", "inter_mean", "inter_std"," silhouette"): 어떤 방법으로 find_optimal_k 돌렸는지
        """
        if method == "inertia":
            plt.figure(figsize=(24,12))
            for val in res.values():
                plt.plot([i for i in range(1,len(val)+1)], val)
            plt.show()
            ...
            for val in res.values():
                plt.plot([i for i in range(1,len(val)+1)], val)
            plt.show()
        elif method == "inter_mean":
            plt.figure(figsize=(24,12))
            for val in res["dist_mean"]:
                plt.plot([i for i in range(1,len(val)+1)], val)
            plt.show()
        elif method == "inter_std":
            plt.figure(figsize=(24,12))
            for val in res["dist_std"]:
                plt.plot([i for i in range(1,len(val)+1)], val)
            plt.show()
        else:
            raise ValueError("method must be one of 'intertia', 'inter_mean', 'inter_std', 'silhouette'")

    @staticmethod
    def visualize_3d_with_answer(
        raw_data,
        fitted_model,
        answer_data,
        demensional_reduction_model = "pca",
